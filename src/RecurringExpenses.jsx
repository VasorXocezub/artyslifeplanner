...
const CARD_COLORS = ['#F5A8C0', '#E31E24', '#3A3A3A', '#3A3A3A', '#3A3A3A', '#E9C86A']
    ...
    <div>
      <div className="view-header">
        <div>
          <p className="view-subtitle">
            {items.length === 0 ? 'Nothing on repeat yet, bestie 🔁' : `${activeItems.length} active and thriving, ${items.length} logged`}
          </p>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <div className="empty-state">
          ...
          <p>Add subscriptions, rent, or bills that repeat, bestie.</p>
        </div>
      )}
          ...
          {items.map((item, i) => {
            ...
            >
              ...
              <div className="habit-actions">
                ...
                >
                  {item.active ? '✓ Active, thriving ✨' : 'Paused — tap to resume'}
                </button>
